...
class SubsService:
    ...
    def _swap_same_position(
        ...
        """
        Same-position swap (G<->G, D<->D, M<->M, F<->F) using the existing
        simple fieldMap logic. This is the simple, single-step swap.
        """
        log.info(
            "[swap:same-pos] Starting: out=%s in=%s period=%s",
            out_player_id, in_player_id, period
        )

        try:
            confirm_resp = self._confirm_swap(
                team_id, out_player_id, in_player_id, explicit_period=period
            )
            log.info("[swap:same-pos] Confirm response: %s", confirm_resp)

            execute_resp = self._execute_swap(
                team_id, out_player_id, in_player_id, explicit_period=period
            )
            log.info("[swap:same-pos] Execute response: %s", execute_resp)
            ...
            if has_error_message or needs_correction:
                log.error(
                    "[swap:same-pos] Failed - msgType=%s, lineupChanges=%s, error: %s",
                    msg_type, len(lineup_changes), main_msg
                )
                return {
                    "success": False,
                    "message": "",
                    "error": main_msg or "Swap failed - check player eligibility and formation rules",
                }

            # Check for lineup changes
            if not lineup_changes:
                log.warning(
                    "[swap:same-pos] No lineupChanges returned. msgType=%s, mainMsg=%s",
                    msg_type, main_msg
                )
                if not main_msg or "success" in main_msg.lower():
                    log.info("[swap:same-pos] Treated as success (no explicit error)")
                else:
                    return {
                        "success": False,
                        "message": "",
            ...
            log.info(
                "[swap:same-pos] Succeeded - %s lineup changes applied",
                len(lineup_changes),
            )
            return {
                "success": True,
            ...
            log.exception("[swap:same-pos] Exception during swap")
            return {
                "success": False,
                "message": "",
                "error": f"Error during same-position swap: {e}",
            }

    def _swap_cross_position(
        self,
        team_id: str,
        out_player_id: str,
        in_player_id: str,
        allow_best_effort: bool = False,
        roster_limit_period: int = None,
    ) -> dict:
        """
        Cross-position swap using the full XI planner (like native UI).

        Semantics: Build new XI = (current starters - OUT) + IN
        Let set_lineup_by_ids handle formation legality, periods, confirm windows.
        """
        if not self.league_id:
            raise ValueError("league_id required for cross-position swaps")

        log.info(
            "[swap:cross-pos] Starting: out=%s in=%s period=%s",
            out_player_id, in_player_id, roster_limit_period
        )
        ...
        if out_player_id not in current_starters:
            return {
                "success": False,
                "message": "",
                "error": "OUT player must currently be in the starting XI.",
            }
        ...
        if in_player_id in current_starters:
            return {
                "success": False,
                "message": "",
                "error": "IN player is already in the starting XI.",
            }

        # Build target XI: current - OUT + IN
        desired = [pid for pid in current_starters if pid != out_player_id]
        desired.append(in_player_id)

        log.info(
            "[swap:cross-pos] Target XI: removing %s, adding %s. Old=%s, New=%s",
            out_player_id, in_player_id, current_starters, desired
        )

        # Use the full XI planner
        plan_result = self.set_lineup_by_ids(
            league_id=self.league_id,
            team_id=team_id,
            desired_starter_ids=desired,
            best_effort=bool(allow_best_effort),
            verify_each=True,
            pos_overrides=None,
        ...
        if not plan_result.get("ok"):
            errs = plan_result.get("errors") or []
            err_msg = "; ".join(errs) if errs else "Cross-position swap failed."
            log.error(
                "[swap:cross-pos] Failed. errors=%s plan=%s",
                errs, plan_result.get("plan_human")
            )
            return {
                "success": False,
                "message": "",
        ...
        formation = plan_result.get("desired_formation", "")
        log.info(
            "[swap:cross-pos] Succeeded. formation=%s starters=%s",
            formation, plan_result.get("current_starters")
        )
        return {
            "success": True,
            "message": f"Swap completed. New formation: {formation}",
            "error": "",
        }

    def swap_players(self, team_id: str, out_player_id: str, in_player_id: str, period: int = None, allow_cross_position: bool = True) -> dict:
        """
        High-level swap API with smart routing.

        Routes to:
        - Same-position swap (G<->G, D<->D, M<->M, F<->F): Simple fieldMap toggle
        - Cross-position swap (e.g. D<->M): Full XI planner (like native UI)

        Returns:
            dict with keys:
                - success (bool): True if swap succeeded
        ...
        if not self.league_id:
            return {
                "success": False,
                "message": "",
                "error": "league_id is required for swaps"
            }

        try:
            # Get roster and identify positions
            roster = self.get_roster(self.league_id, team_id)
            row_map = self._row_map(roster)
            
            out_row = row_map.get(out_player_id)
            in_row = row_map.get(in_player_id)

            if not out_row or not in_row:
                missing = []
                if not out_row:
                    missing.append(f"OUT player {out_player_id}")
                if not in_row:
                    missing.append(f"IN player {in_player_id}")
                return {
                    "success": False,
                    "message": "",
                    "error": " / ".join(missing) + " not found on roster.",
                }

            # Determine position buckets (G/D/M/F)
            out_pos = self._pos_of_row(out_row, None)
            in_pos = self._pos_of_row(in_row, None)

            out_name = getattr(out_row.player, "name", "?")
            in_name = getattr(in_row.player, "name", "?")

            log.info(
                "[swap] Request: OUT=%s(%s) IN=%s(%s) period=%s",
                out_name, out_pos, in_name, in_pos, period
            )

            # Route 1: Same position bucket → simple swap
            if out_pos == in_pos and out_pos in {"G", "D", "M", "F"}:
                log.info("[swap] Detected same-position swap (%s <-> %s), using simple path", out_pos, in_pos)
                return self._swap_same_position(team_id, out_player_id, in_player_id, period)

            # Route 2: Cross-position → full XI planner
            log.info("[swap] Detected cross-position swap (%s <-> %s)", out_pos, in_pos)
            ...
            if not allow_cross_position:
                return {
                    ...
                    "message": "",
                    "error": (
                        f"Cross-position swaps are not allowed via this endpoint "
                        f"(OUT={out_pos}, IN={in_pos}). Enable allow_cross_position=True."
                    ),
                }

            return self._swap_cross_position(
                team_id=team_id,
                out_player_id=out_player_id,
                in_player_id=in_player_id,
                allow_best_effort=False,
                roster_limit_period=period,
            )

        except Exception as e:
            log.exception("[swap] Exception during swap routing")
            return {
                "success": False,
                "message": "",
    ...
    def _build_swap_field_map(self, team_id: str, out_id: str, in_id: str) -> dict:
        ...
        Simple swap logic:
        1. Build full fieldMap from current roster state
        2. Toggle stId for OUT (starter → bench) and IN (bench → starter)
        3. Keep each player's actual posId - don't force position changes
        4. Let Fantrax validate formation legality
        
        Fantrax fieldMap format:
        - Keys: player IDs (strings like "04tm0")
        - Values: {"posId": "703", "stId": "1"} with STRING values
        - stId: "1" = starter, "2" = bench
        - posId: position slot ID (701=F, 702=M, 703=D, 704=GK, 0=bench)
        """
        roster = self.get_roster(self.league_id, team_id)
        
        # Build full map from current state
        field_map = {}
        out_row = None
        in_row = None
        ...
        for row in roster.rows:
            ...
            # Determine stId from current state
            if pos_id == "0":
                st_id = "2"  # Bench
            else:
                st_id = "1"  # Starter
        ...
        # Validate: OUT must be starter, IN must be bench
        if field_map[out_id]["stId"] != "1":
            raise ValueError(f"OUT player {out_row.player.name} must be a starter (currently stId={field_map[out_id]['stId']})")
        if field_map[in_id]["stId"] != "2":
            raise ValueError(f"IN player {in_row.player.name} must be on bench (currently stId={field_map[in_id]['stId']})")
        
        # Get current positions
        out_pos_id = field_map[out_id]["posId"]
        in_pos_id = field_map[in_id]["posId"]
        
        # SIMPLE SWAP: Just toggle stId, keep posId unchanged
        field_map[out_id]["stId"] = "2"  # Move to bench
        field_map[in_id]["stId"] = "1"   # Move to starters
        
        # Special case: If IN player has posId=0 (generic bench), assign their actual position
        # This happens when bench players don't have position slots assigned yet
        if in_pos_id == "0":
            # Try to get their primary position from player data
            in_primary = getattr(in_row.player, "position", "") or ""
            in_positions = getattr(in_row.player, "positions", []) or []
            
            # Map position letters to posId (this is league-specific, but common mapping)
            # GK=704, D=703, M=702, F=701
            pos_map = {"GK": "704", "D": "703", "M": "702", "F": "701", "G": "704"}
            
            # Try primary position first
            if in_primary in pos_map:
                field_map[in_id]["posId"] = pos_map[in_primary]
                log.info(f"[swap] Set IN player {in_row.player.name} posId to {pos_map[in_primary]} based on primary position '{in_primary}'")
            elif in_positions and in_positions[0] in pos_map:
                field_map[in_id]["posId"] = pos_map[in_positions[0]]
                log.info(f"[swap] Set IN player {in_row.player.name} posId to {pos_map[in_positions[0]]} based on eligible position '{in_positions[0]}'")
            else:
                # No position data - bail out instead of guessing
                raise ValueError(
                    f"IN player {in_row.player.name} has no position data (posId=0, primary='{in_primary}', eligible={in_positions}). "
                    "Cannot determine valid position slot. Please refresh roster data or fix position parsing."
                )
        
        log.info(f"[swap] Built full fieldMap with {len(field_map)} players")
        log.info(f"[swap] Simple swap: OUT={out_row.player.name} (posId={out_pos_id}) stId 1→2 (bench), IN={in_row.player.name} (posId={field_map[in_id]['posId']}) stId 2→1 (starter)")
        log.debug(f"[swap] fieldMap sample: {dict(list(field_map.items())[:3])}")
