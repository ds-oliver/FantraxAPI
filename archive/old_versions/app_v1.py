...
def _apply_xi_server_confirm(
	*,
	...
	"""Apply lineup changes one swap at a time."""
	
	# Get current state
	current = lineup.get_roster(league_id, team_id)
	current_ids = [r.player.id for r in current.get_starters() if getattr(r, "player", None)]
	logger.info(f"[apply_xi] Current starters: {current_ids}")
	logger.info(f"[apply_xi] Desired starters: {desired_ids}")
	
	# Calculate needed changes
	to_remove = set(current_ids) - set(desired_ids)
	to_add = set(desired_ids) - set(current_ids)
	logger.info(f"[apply_xi] Players to remove: {to_remove}")
	logger.info(f"[apply_xi] Players to add: {to_add}")
	
	# Create swap pairs
	changes = list(zip(to_remove, to_add))
	logger.info(f"[apply_xi] Planned changes: {changes}")
	
	# Get player names for better logging
	name_map = {}
	for row in current.rows:
		if getattr(row, "player", None):
			name_map[row.player.id] = row.player.name
	
	for out_id, in_id in changes:
		logger.info(f"[apply_xi] Planning swap: {name_map.get(out_id, out_id)} -> {name_map.get(in_id, in_id)}")
	
	# Apply changes sequentially
	success = lineup.apply_changes(league_id, team_id, changes)
	logger.info(f"[apply_xi] Changes applied successfully: {success}")
	
	if success:
		# Verify final state
		final = lineup.get_roster(league_id, team_id)
		final_ids = [r.player.id for r in final.get_starters() if getattr(r, "player", None)]
		logger.info(f"[apply_xi] Final starters: {final_ids}")
		...
		# Get current state after failure
		failed_state = lineup.get_roster(league_id, team_id)
		failed_ids = [r.player.id for r in failed_state.get_starters() if getattr(r, "player", None)]
		logger.error(f"[apply_xi] Failed state starters: {failed_ids}")
		
		return {
			"ok": False,
			"warnings": [],
			"errors": ["Failed to apply all changes"]
		}
...
def ui_leagues_and_rosters_section():
	...
	if show_all_rosters:
		...
		lineup = LineupService(session, league_id=league_id)
		current = lineup.get_roster(league_id, team_id)

		# IMPORTANT: warm bench eligibilities so they appear in pickers
		try:
			lineup.warm_codes_for_roster(league_id, current)
		except Exception as _e:
			logger.warning(f"warm_codes_for_roster failed (non-fatal): {_e}")
		...
		c1, c2 = st.columns(2)
		with c1:
			pick_G = st.multiselect("Goalkeepers (0 to 1)", options=pool_by_pos["G"], default=pre_G[:1], max_selections=1)
		...
		apply_future = st.checkbox("Apply to future period if deadline has passed", value=True)
		apply_btn = st.button("Apply XI", type="primary", disabled=(total != 11))

		if apply_btn:
			with st.spinner("Submitting XI via Fantrax confirm flow…"):
				try:
					# Log current state and desired changes
					logger.info("Current starters:")
					for r in current.get_starters():
						if getattr(r, "player", None):
							logger.info(f"	{r.pos.short_name or 'BN'} • {r.player.name} ({r.player.id})")

					logger.info("Desired XI:")
					for pid in desired_ids:
						row = next((r for r in all_rows if getattr(r, "player", None) and r.player.id == pid), None)
						if row:
							pos = pos_overrides.get(pid, getattr(getattr(row, "pos", None), "short_name", "BN"))
							logger.info(f"	{pos} • {row.player.name} ({pid})")

					# Quick local diff against the roster shown above to avoid no-op submits
					cur_start_ids = [r.player.id for r in current.get_starters() if getattr(r, "player", None)]
					logger.info(f"Local diff - current_ids: {cur_start_ids}")
					logger.info(f"Local diff - desired_ids: {desired_ids}")
					logger.info(f"Local diff - pos_overrides: {pos_overrides}")

					if set(cur_start_ids) == set(desired_ids):
						st.info("No changes detected: selected XI matches the current starters shown above.")
						st.stop()

					# optional: still run local preflight to catch obvious UI mistakes early
					...
					logger.info(f"Preflight result: {pre}")

					if pre.get("errors"):
						st.error(" • ".join(pre["errors"]))
						st.stop()

					logger.info("Submitting lineup changes to server...")
					res = _apply_xi_server_confirm(
						lineup=lineup,
						dropsvc=service,
						league_id=league_id,
						team_id=team_id,
						...
						pos_overrides=pos_overrides,
						apply_to_future=bool(apply_future),
					)
					logger.info(f"Server response: {res}")

					if res.get("warnings"):
						st.warning(" • ".join([str(w) for w in res["warnings"]]))

					if res.get("ok"):
						logger.info("Lineup change successful")
						st.success("Lineup applied.")
						st.rerun()
					else:
						logger.error("Lineup change failed")
						if res.get("finalize"):
							fr = res["finalize"].get("fantasyResponse", {})
							logger.error(f"Server response details: type={fr.get('msgType')}, mainMsg={fr.get('mainMsg')}")
							logger.error(f"Illegal messages: {fr.get('illegalRosterMsgs')}")
							model = (fr.get("textArray") or {}).get("model", {})
							logger.error(f"Model state: changeAllowed={model.get('changeAllowed')}, deadlinePassed={model.get('playerPickDeadlinePassed')}")
						st.error("Lineup change failed.")
						# surface server message if available
						main = ((res.get("finalize") or {}).get("fantasyResponse") or {}).get("mainMsg")
						if main:
							st.error(str(main))
						illegal = ((res.get("finalize") or {}).get("fantasyResponse") or {}).get("illegalRosterMsgs") or []
						for m in illegal:
							st.write(f"• {m}")
				except Exception as e:
					logger.exception("Apply XI failed")
					st.error(str(e))
