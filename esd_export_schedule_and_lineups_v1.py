#!/usr/bin/env python3
"""
Export SofaScore schedule + lineups via EasySoccerData (ESD), with raw-HTTP fallbacks.
...
Examples:
  ...
  python esd_export_schedule_and_lineups.py --upcoming --with-lineups
...
def raw_get_json(url: str, referer: Optional[str] = None) -> dict:
    ...
    with httpx.Client(follow_redirects=True, headers=headers, timeout=30) as s:
        ...
        r.raise_for_status()
        # Some edges return gzipped JSON correctly; httpx handles it.
        return r.json()
...
def pick_season_id(client: "esd.SofascoreClient", tournament_id: int,
    ...
    """
    Resolve season_id using ESD first; on failure, use raw HTTP.
    """
    ...
    if seasons_esd:
        return choose_season_from_list(seasons_esd, season_text)
    ...
    return choose_season_from_list(seasons_raw, season_text)

def season_label(s) -> str:
    return str(
        getv(s, "year", "name", default="")
    )

def season_is_current(s) -> bool:
    cur = getv(s, "current", default=False)
    return bool(cur)
...
def main():
    ap = argparse.ArgumentParser(description="Export SofaScore schedule + lineups using EasySoccerData with raw fallbacks.")
    ap.add_argument("--tournament-id", type=int, default=17, help="SofaScore tournament id (default: 17 = Premier League)")
    ap.add_argument("--season", type=str, default=None, help="Season label to match (e.g. '2024/2025' or '24/25')")
    ...
    # Resolve season id (ESD -> raw fallback)
    ...
    # Collect and save schedule (ESD -> raw fallback handled inside iterator)
    rows = []
    for ev in iter_tournament_events(client, args.tournament_id, season_id, upcoming=args.upcoming):
    ...
    # Fetch lineups per event (ESD first, then raw)
    saved = []
    for r in rows:
